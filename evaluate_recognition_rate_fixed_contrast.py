...
def evaluate_recognition_rate(fg_masks, bg_masks, digit, noise_variance, trials=100):
    correct_count = 0
    for _ in range(trials):
        # генерация изображения с шумом
        image_path = os.path.join(input_folder, f"{digit}.bmp")
        fg_mask, bg_mask = get_foreground_background(image_path)
        noisy_image = create_image_with_noise(fg_mask, bg_mask, fg_brightness=0, bg_brightness=255, noise_variance=noise_variance)
...
if __name__ == "__main__":
    ...
    # определение диапазона дисперсий
    noise_variances = [10, 100,  1000, 10000, 100000, 1000000, 10000000, 100000000]
    recognition_rates = []

    for variance in noise_variances:
        correct_rate = evaluate_recognition_rate(fg_masks, bg_masks, digit=random.randint(0, 9), noise_variance=variance, trials=100)
        recognition_rates.append(correct_rate)

    # построение графика зависимости частоты правильного распознавания от дисперсии
    plt.figure(figsize=(10, 5))
    plt.plot(noise_variances, recognition_rates, marker='o')
    plt.title('График зависимости частоты правильного распознавания от дисперсии')
